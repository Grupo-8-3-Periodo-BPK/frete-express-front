...
function DriverContractsPage() {
  ...
  const handleDelete = async (contractId) => {
    if (window.confirm("Tem certeza que deseja retirar sua proposta?")) {
      try {
        ...
        setAlert({ isAlertOpen: true, message: "Proposta retirada.", type: "success" });
        fetchContracts();
      } catch (err) {
        setAlert({ isAlertOpen: true, message: "Erro ao retirar a proposta.", type: "error" });
      }
    }
  };

  // --- Componentes de Estado ---
  const LoadingState = () => <div className="flex flex-col items-center justify-center pt-24 text-center"><Loader className="h-12 w-12 animate-spin text-emerald-500" /><p className="mt-4 text-lg">Carregando seus fretes...</p></div>;
  ...
  const EmptyState = () => <div className="text-center py-16 px-6 rounded-lg bg-white dark:bg-gray-800/50 border-dashed border-gray-300 dark:border-gray-700"><Truck className="mx-auto h-16 w-16 text-gray-400" /><h2 className="mt-6 text-xl font-semibold">Nenhuma proposta de frete</h2><p className="mt-2 text-gray-500 dark:text-gray-400">Novas oportunidades e contratos aceitos serão listados aqui.</p></div>;
    ...
    <div className="w-full bg-gray-50 dark:bg-gray-900 text-gray-800 dark:text-gray-200 min-h-screen">
      <header className="bg-white dark:bg-gray-800/50 border-b border-gray-200 dark:border-gray-700/50 shadow-sm">
        <div className="max-w-5xl mx-auto py-6 px-4 sm:px-6">
          <h1 className="text-3xl font-bold tracking-tight text-gray-900 dark:text-white">Seus Fretes</h1>
          <p className="mt-1 text-gray-500 dark:text-gray-400">Gerencie suas propostas e contratos ativos.</p>
        </div>
      </header>
          ...
          contracts.length > 0 ? (
            <div className="space-y-5">
              {contracts.map((contract) => (
                <div key={contract.id} className="rounded-lg border-l-4 border-emerald-500 overflow-hidden transition-all hover:-translate-y-1">
                  <ContractCard
                    contract={contract}
                    darkMode={darkMode}
                    userRole="DRIVER"
